const functions = require('./parte-I');
const { sum, myRemove, myRemoveWithoutCopy, myFizzBuzz, obj1, obj2, obj3, obj4 } = functions;

describe('Testa a função sum', () => {
  it('Teste se o retorno de sum(4, 5) é 9', () => {
    expect(sum(4, 5)).toEqual(9);
  });

  it('Teste se o retorno de sum(0, 0) é 0', () => {
    expect(sum(0, 0)).toEqual(0);
  });

  it(`Teste se a função sum lança um erro quando os parâmetros são 4 e '5' (5 string)`,() => {
    expect(() => { sum(4, '5') }).toThrow();
  });

  it(`Teste se a mensagem de erro é "parameters must be numbers" quando realiza a chamada sum(4, '5')`, () => {
    expect(() => { sum(4, '5') }).toThrowError(new Error('parameters must be numbers'));
  });
});

describe('Testa a função myRemove', () => {
  it('Verifique se a chamada myRemove([1, 2, 3, 4], 3) retorna [1, 2, 4]', () => {
    expect(myRemove([1, 2, 3, 4], 3)).toEqual([1, 2, 4]);
  });

  it('Verifique se a chamada myRemove([1, 2, 3, 4], 3) não retorna o array [1, 2, 3, 4]', () => {
    expect(myRemove([1, 2, 3, 4], 3)).not.toEqual([1, 2, 3, 4]);
  });
  it('Verifique se o array passado por parâmetro não sofreu alterações', () => {
    const array = [1, 2, 3, 4];
    myRemove(array, 4);
    expect(array).toEqual([1, 2, 3, 4]);
  });
  it('Verifica se a chamada myRemove([1, 2, 3, 4], 5) retorna o array esperado', () => {
    expect(myRemove([1, 2, 3, 4], 5)).toEqual([1, 2, 3, 4]);
  });
});

describe('Testa a função myRemoveWithoutCopy', () => {
  it('Verifique se a chamada myRemoveWithoutCopy([1, 2, 3, 4], 3) retorna o array esperado', () => {
    expect(myRemoveWithoutCopy([1, 2, 3, 4], 3)).toEqual([1, 2, 4]);
  });

  it('Verifique se a chamada myRemoveWithoutCopy([1, 2, 3, 4], 3) não retorna o array [1, 2, 3, 4]', () => {
    expect(myRemoveWithoutCopy([1, 2, 3, 4], 3)).not.toEqual([1, 2, 3, 4]);
  });

  it('Faça uma chamada para função myRemoveWithoutCopy e verifique se o array passado por parâmetro sofreu alterações', () => {
    const array = [1, 2, 3, 4];
    myRemoveWithoutCopy(array, 3);
    expect(array).toContain(1, 2, 4);
  });
  
  it('Verifique se o chamada myRemoveWithoutCopy([1, 2, 3, 4], 5) retorna o array esperado', () => {
    expect(myRemoveWithoutCopy([1, 2, 3, 4], 5))
    .toEqual([1, 2, 3, 4]);
  })
});

describe('Testa função myFizzBuzz', () => {
  it(`Faça uma chamada com um número divisível por 3 e 5 e verifique se o retorno é o 'fizzbuzz'`, () => {
    expect(myFizzBuzz(15)).toEqual('fizzbuzz');
  });

  it(`Faça uma chamada com um número divisível por 3 e verifique se o retorno é 'fizz'`, () => {
    expect(myFizzBuzz(9)).toEqual('fizz');
  });

  it(`Faça uma chamada com um número divisível por 5 e verifique se o retorno é 'buzz'`, () => {
    expect(myFizzBuzz(10)).toEqual('buzz');
  });

  it(`Faça uma chamada com um número que não é divisível por 3 ou 5 e verifique se o retorno é o esperado`, () => {
    expect(myFizzBuzz(7)).toEqual(7);
  })

  it(`Faça uma chamada com um parâmetro que não é um número e verifique se o retorno é o esperado`, () => {
    expect(myFizzBuzz('5')).toEqual(false);
  });
});

describe('Testa objetos', () => {

  it(`Verifica se os objetos obj1 e obj2 são iguais`, () => {
    expect(obj1).toEqual(obj2);
  });

  it(`Verifica se os objetos obj1 e obj3 são diferentes`, () => {
    expect(obj1).not.toEqual(obj3)
  });

  it(`Verifica se os objetos obj1 e obj4 são os mesmos`, () => {
    expect(obj1).toBe(obj4);
  });
});
