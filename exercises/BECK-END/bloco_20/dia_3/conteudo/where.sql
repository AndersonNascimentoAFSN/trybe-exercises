...
SELECT * FROM sakila.customer WHERE active = 0 AND store_id = 2 AND NOT first_name='KENNETH' ORDER BY first_name;
...
SELECT title, description, release_year, replacement_cost, rating FROM sakila.film WHERE rating='G' AND replacement_cost >= 18 ORDER BY replacement_cost DESC, title ASC LIMIT 100;

-- Quantos clientes estão ativos e na loja 1 ?
SELECT COUNT(*) AS 'Clientes ativos loja 1' FROM sakila.customer WHERE store_id=1 AND active=1;

-- Mostre todos os detalhes dos clientes que não estão ativos na loja 1 .
SELECT * FROM sakila.customer WHERE active=false AND store_id=1;
...
SELECT * FROM sakila.film WHERE rating="R" ORDER BY rental_rate ASC, title ASC LIMIT 50;
